...
PIXELML_MODEL_WEIGHTS_DIR = PurePath("/data") / ".pixelml" / "model_weights"

_REQUEST_HEADERS = {"User-Agent": f"pixelml-client (python)"}
...
def download_model_weights(url: str, force: bool = False) -> Path:
    ...
    return download_model_weights_pixelml(url, request_headers=headers, force=force)


def download_model_weights_pixelml(
    url: str, force: bool = False, request_headers: dict[str, str] | None = None
) -> Path:
    weights_dir = Path(PIXELML_MODEL_WEIGHTS_DIR / _hash_url(url))
